...
Let's start by loading tidyverse and reading in the Maryland PPP loan applications data:
...
```{r}
maryland %>% left_join(naics_codes, by="naics_code") %>% select(name, naics_code, title) %>% nrow()
```
...
```{r}
maryland %>% left_join(naics_codes) %>% select(name, naics_code, title)
```
...
```{r}
maryland_with_title <- maryland %>% left_join(naics_codes)
```
...
```{r}
maryland_with_title_and_demographics <- maryland_with_title %>% left_join(maryland_zcta, by=c("zip"="ZCTA5N"))
```
...
```{r}
maryland_with_title_and_demographics <- maryland_with_title %>% left_join(maryland_zcta, by=c("zip"="ZCTA5N"))
View(maryland_with_title_and_demographics)
```
...
```{r}
maryland_with_title <- maryland %>% mutate(zip5 = str_sub(zip, 1, 5))
maryland_with_title_and_demographics <- maryland_with_title %>% left_join(maryland_zcta, by=c("zip5"="ZCTA5N"))
```
...
```{r}
maryland_with_title_and_demographics %>%
  filter(PNHB > 50) %>%
  summarize(
    count = n(),
