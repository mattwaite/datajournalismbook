...
Select Spreadsheet, but know that it's a lie. If you try to load it using read_excel, you'll get an error. Why? Because what is being downloaded is an HTML file. So you can use `rvest` to read and parse it.
...
If you open `cleanclaims`, you may notice something, depending on when you do this analysis:

```{r}
cleanclaims %>% arrange(desc(filed_week_ended)) %>% head()
```

See it? The latest data isn't in there in my version. [It's in a press release](https://www.dol.gov/sites/dolgov/files/OPA/newsreleases/ui-claims/20200510.pdf).

So how do we add it? We use `add_row` from the tidyverse (specifically the `tibble` library). 

```{r}
updatedcleanclaims <- cleanclaims %>% add_row(state="Nebraska", filed_week_ended=as.Date("2020-03-21"), initial_claims=15688, reflecting_week_ended=as.Date("2020-03-14"))
```
...
ggplot() + 
  ...
  geom_text(data=fedlatest, aes(x=filed_week_ended-200, y=initial_claims + 500, label="15,668")) + 
  geom_smooth(data=fedranked, aes(x=filed_week_ended, y=initial_claims), method=loess, se=FALSE) + 
  labs(title="A record for jobless claims in Nebraska", subtitle="Applications for unemployment jumped 1,871 percent from the week ending March 14 to the 21st. The worst is yet to come.", x="Date", y="Claims", caption = "Source: US Dept. of Labor  |  Graphic by Matt Waite") +
  theme_minimal() + 
  theme(
    plot.title = element_text(size = 16, face = "bold"),
    axis.title = element_text(size = 8), 
