...
```{r}
change <- round((((fedlatest$initial_claims - fedprevious$initial_claims)/fedprevious$initial_claims)*100), digits=2)
```

Then the change from the pre-crisis week.

```{r}
prechange <- round((((fedlatest$initial_claims - pre$initial_claims)/pre$initial_claims)*100), digits=2)
```
...
ggplot() + 
  ...
  geom_text(data=fedlatest, aes(x=filed_week_ended-200, y=initial_claims + 500, label=initial_claims)) + 
