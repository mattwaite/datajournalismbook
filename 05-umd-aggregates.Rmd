...
The CSV file we're going to read in is from the [Small Business Administration's Paycheck Protection Program(https://www.sba.gov/funding-programs/loans/covid-19-relief-options/paycheck-protection-program)]. The program gave loans to businesses to keep people employed during the Covid-19 pandemic. We'll be working with a slice of the data that documents loans to Maryland businesses. 

So step 1 is to import the data. The code to import the data looks like this, but hold off copying it just yet:

`ppp_maryland_loans <- read_csv("ppp_maryland.csv")`

Let's unpack that.

The first part -- **ppp_maryland_loans** -- is the name of a variable. 

A **variable** is just a name that we'll use to refer to some more complex thing. In this case, the more complex thing is the data we're importing into R that will be stored as a **dataframe**, which is one way R stores data. We can call this whatever we want. 

The variable name doesn't matter, technically.  We could use any word.  You could use your first name, if you like. Generally, though, we want to give variables names that are descriptive of the thing they refer to. Which is why we're calling this one **ppp_maryland_loans**. Variable names, by convention are one word all lower case (or two or more words connected by an underscore). You can end a variable with a number, but you can't start one with a number.
...
**read_csv()** is a function, one that only works when we've loaded the tidyverse.  A **function** is a little computer program that takes in information and follows a series of pre-determined steps and spits it back out.  A recipe to make pizza is a kind of function.  You take a set of ingredients -- flour, water, oil, cheese, tomato, salt -- and pass them through each step outlined in a recipe -- mix flour and water and oil, knead, let it sit, roll it out, put tomato sauce and cheese on it, bake it in an oven, then take it out.  The output of our "make pizza" function is a finished pie. 
...
The easiest thing to do, if you are confused about how to find your data, is to put your data in the same folder as as your notebook (you'll have to save that notebook first). If you do that, then you just need to put the name of the file in there (ppp_maryland.csv). If you put your data in a folder called "data" that sits next to your data notebook, your function would instead look like this: 

```{r}

#ppp_maryland_loans <- read_csv("ppp_loan_data/ppp_maryland.csv")

ppp_maryland_loans <- read_rds("in_class_demonstrations/class_01_demonstration/class_01_demonstration.rds")
...
```
If we type `summary(ppp_maryland_loans)`, it will print out a list of columns, column types, and a summary of the values in number columns (mean, median, other values). 

```{r}

summary(ppp_maryland_loans)
...
As you can see here, it's not clear that we'll be able to make use of the race data going forwardds.  By far the most common category in every county is "Unanswered". We'll avoid grouping by race in the rest of the examples in this chapter. 
